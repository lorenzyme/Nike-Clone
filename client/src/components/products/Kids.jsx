...
const Kids = () => {
    ...
    const addProductToCart = async (productId) => {
        ...
        const kidsReponse = await fetch('http://localhost:3000/nike/cartItem/new', {
            headers: {
                authorization: token,
        ...
        // console.log(productId)
        const kidsData = await kidsReponse.json()
    ...
    const updateProductInCart = async (productId) => {
        const token = window.localStorage.getItem('token')
        const kidsReponse = await fetch(`http://localhost:3000/nike/cartItem/${productId}`, {
            headers: {
                authorization: token,
                "Content-Type": "application/json"
            },
        ...
        const updatedKidsData = await kidsReponse.json()
        const cartItems = await axios.get("http://localhost:3000/nike/cartItem", {
          headers: {
            authorization: token,
          },
